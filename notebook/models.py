...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.shape[0], self.x_size, -1).transpose(1,2) #先升维然后调换后两个维度顺序
        x = x.to(self.device)
        ...
        h0 = torch.zeros(self.num_layers, x.shape[0], self.hidden_size, device=self.device)
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        h_0 = torch.zeros(self.num_layers, x.shape[0], self.hidden_size, device=self.device)
        c_0 = torch.zeros(self.num_layers, x.shape[0], self.hidden_size, device=self.device)
...
class TCN(nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout, device='cuda'):
        ...
        self.device = device
        self.tcn = TemporalConvNet(self.input_size, num_channels, kernel_size=kernel_size, dropout=dropout)
...
class EnCQR():
    ...
    def fit(self, X_train, Y_train):
        ...
        for b in range(self.n_ensemble):
            print('-- training: ' + str(b+1) + ' of ' + str(self.n_ensemble) + ' NNs --')
